...
const Post = require("../models/board"); //스키마 폴더 안에 board 스키마
...
router.get("/posts", async (req, res) => { 
    ...
    const board = await Post.find({ postDate }).sort({ postDate: -1 }); //Board라는 스키마에서 find, sort()날짜기준 내림차순 
...
router.post("/write", authMiddleware, async (req, res) => { 
    try {
        ...
        const createBoard = await Post.create({ postId, nickname: user.nickname, title, content, postDate });
...
router.put("/posts/:postId", authMiddleware, async (req, res) => { // 게시글 수정 API
    const { user } = res.locals;
    const { postId } = req.params;
    const { password, title, content } = req.body;
    const correctPw = await Post.findOne({user})
    if (password === correctPw.password) {
        const updateBoard = await Post.updateOne({ postId: Number(postId) }, { $set: { title, content } });
...
router.delete("/posts/:postId", authMiddleware, async (req, res) => { // 게시글 삭제 API
    const { user } = res.locals;
    const { postId } = req.params;
    const { password } = req.body;
    const correctPw = await Post.findOne({ user })
    if (password == correctPw.password) {
        const deleteBoard = await Post.deleteOne({ postId: Number(postId) });
